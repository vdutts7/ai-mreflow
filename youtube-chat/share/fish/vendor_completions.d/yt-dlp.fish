...
complete --command yt-dlp --long-option update-to --description 'Upgrade/downgrade to a specific version. CHANNEL and TAG defaults to "stable" and "latest" respectively if omitted; See "UPDATE" for details. Supported channels: stable, nightly'
...
complete --command yt-dlp --long-option no-match-filter --description 'Do not use any --match-filter (default)'
